...
class DescriptionMapper constructor(
    ...
    fun map(domain: MediaDomain, selectedPlaylists: Set<PlaylistDomain>?, editablePlaylists: Boolean = true): DescriptionModel =
        DescriptionModel(
            title = domain.title,
            ...
            chips = mutableListOf<ChipModel>()
                .apply { if (editablePlaylists) add(ChipModel.PLAYLIST_SELECT_MODEL) }
                .apply {
                    selectedPlaylists
                        ?.forEachIndexed { index, playlist -> add(index, chipModel(playlist, editablePlaylists)) }
                },
            pubDate = dateTimeFormater.formatDateTimeNullable(domain.published),
        )

    private fun chipModel(playlist: PlaylistDomain, editablePlaylists: Boolean) =
        ChipModel(ChipModel.Type.PLAYLIST, playlist.title, playlist.id.toString(), playlist.thumb ?: playlist.image, editablePlaylists)
